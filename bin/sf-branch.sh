...
# sf-init.sh から呼ばれた場合はブランチのリモート作成をスキップ
# （この時点では初回コミットがないため push できない。Phase 8 で実施する）
# SF_INIT_MODE は sf-init.sh 本体で export されており、すべてのフェーズスクリプトに継承される
if [[ "${SF_INIT_MODE:-0}" == "1" ]]; then
    log "INFO" "sf-init.sh から実行中のため、ブランチのリモート作成は Phase 8 で実施します。"
    exit $RET_OK
fi
