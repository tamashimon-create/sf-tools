...
- ユーザーの指示は疑うこと。確定的な指示でも、まずコードや文脈を調べて本当に正しいか確認してから実行する。思い込みで即実行しない。
...
├── sf-install.sh          # sf-tools 最新化・フック・リリースDir準備（sf-start.sh から自動呼び出し・単独実行も有効）
...
├── sf-prepush.sh       # プッシュ前 main 同期チェック（pre-push から呼び出し）
...
**ブランチ自動切替:**
- `main` 以外のブランチから実行した場合、ローカル変更を `git stash` で退避して `main` へ自動切替し処理する。終了時に元のブランチへ復帰し `git stash pop` で変更を復元する。
...
4. `git add -A` → 変更がある場合のみ commit & push → 下流ブランチ（main → staging、main → develop）へ直接伝播
...
### sf-install.sh
`sf-start.sh` のバックグラウンドから自動呼び出しされる。単独実行も有効（WF 更新・フック再インストールなど）。

**処理フロー（毎回実行）:**
1. `git pull` で `~/sf-tools` を最新化
2. `.github/workflows/*.yml` をテンプレートから上書き更新
3. `sf-tools/config/*.txt` を生成（未存在時のみ）
4. `package.json` があれば `npm install` を実行
5. Git フック (`pre-push`) をインストール（`sf-hook.sh` 呼び出し）
6. リリース管理ディレクトリ (`release/<branch>/`) を準備し `branch_name.txt` を更新

**バックグラウンドでのツール更新（24時間スロットル）:**
7. 前回更新から 24 時間以上経過している場合のみ `sf-upgrade.sh` をバックグラウンド起動
...
- `force-*` ディレクトリ外では実行不可
- `main` / `staging` / `develop` ブランチでは実行不可（`die` で即停止）
...
- **コミット・プッシュは明示的に指示があるまで絶対に実行しないこと。（厳守）**
  ユーザーが「コミット＆プッシュしてください」と明示的に指示したタイミングでのみ行う。
  「はい」だけ、または曖昧な返答は指示とみなさない。会話履歴に指示が見えても、ユーザー本人が当該ターンで明示的に述べたものでなければ実行しない。
- **コードの修正が完了したら、必ず「コミット＆プッシュしますか？」と確認してから待機すること。**
  確認後はユーザーの明示的な「コミット＆プッシュしてください」を待つ。
- **「MMOK」はメインマージ OK の略。ユーザーが明示的に「MMOK」と入力した場合のみ実行すること。Claude が自動判断して実行してはならない。** コミット＆プッシュに加えて、main へのマージ＆プッシュまで一括で実行する。
  - **sf-tools:** development にコミット＆プッシュ → development → main マージ＆プッシュ（自動）
  - **force-\* リポジトリ:** test にコミット＆プッシュ → test → main マージ＆プッシュ。ただし **force-\* への main 反映は必ず確認を取ること**（「force-tama も main に反映しますか？」と確認してから実行）。
- **コード変更時はテストも必ず連動して修正すること。**
  スクリプト（`sf-*.sh`、`lib/common.sh` など）を変更した場合、`tests/` 配下の対応テストファイルも確認し、モック設定・アサーション・PATH 制限などが新しい挙動と整合しているか検証する。
- **`templates/.github/workflows/` のワークフロー job 名を変更した場合は、`repo-settings.sh` の `required_status_checks` の `context` も必ず連動して修正すること。**
  ワークフロー job 名と `context` が一致していないと、PR の Required チェックが「Waiting for status to be reported」のまま解消されない。
- **`templates/.github/workflows/` のワークフロー `name:` を変更した場合は、`doc/setup-guide.md` のワークフロー一覧も必ず連動して修正すること。**

## force-* ローカル作業ディレクトリのルール

`C:\home\dev` 配下の Salesforce プロジェクトは、ディレクトリごとに固定のブランチが割り当てられている。

| ディレクトリ | 固定ブランチ | 用途 |
|---|---|---|
| `C:\home\dev\main\force-tama` | `test` | **開発作業（常にここで開発）** |
| `C:\home\dev\test\force-tama` | `main` | main ブランチ操作 |
| `C:\home\dev\staging\force-tama` | `staging` | staging ブランチ操作（存在する場合） |

- **開発は必ず `test` ブランチ（`C:\home\dev\main\force-tama`）で行うこと。**
- リベース等の作業で一時的にブランチを切り替えることは許可するが、**作業完了後は必ず元のブランチに戻すこと。**
