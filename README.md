...
| 🔍 `sf-mergecheck.sh` | プッシュ前 main 同期チェック | pre-push フックから自動呼び出し（通常は直接実行不要） |
...
### 🔍 sf-mergecheck.sh — プッシュ前 main 同期チェック
...
├── sf-mergecheck.sh           # 🔍 プッシュ前 main 同期チェック（pre-push から自動呼び出し）
