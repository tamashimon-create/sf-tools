...
`release/<branch>/deploy-target.txt` と `remove-target.txt` を更新します（書き方は [5. デプロイ対象ファイルの書き方](#5-デプロイ対象ファイルの書き方) を参照）。
...
```
  ...
  >> sf-tools ランチャー
  ──────────────────────────────────────────────────
  [1] Check      deploy-target.txt の構文チェック
  [2] Push       変更をコミット＆プッシュ
  [3] Next       次の PR 先ブランチを確認
  [4] Dryrun     現在接続中のSandboxへリリース検証
  [5] Deploy     現在接続中のSandboxへリリース
  [6] Start      開発環境を起動（ログイン・VSCode・フック設定）
  [7] Restart    接続組織を切り替えて再起動
  ──────────────────────────────────────────────────
  番号を入力 (1-7 / q で終了):
```
