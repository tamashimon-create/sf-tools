...
`force-XXXX` 側に設置する caller WF（`wf-validate.yml` / `wf-release.yml` 等）は、
`sf-init` 実行時に `sf-tools/templates/` からコピーして配布する。

実運用開始後に caller WF を変更・追加・削除する必要が生じた場合、
...
sf-tools/
├── .github/workflows/
│   ├── wf-validate-reusable.yml  ← ロジックの実体（@main 参照で自動更新）
│   └── wf-release-reusable.yml   ← ロジックの実体（@main 参照で自動更新）
└── templates/.github/workflows/  ← caller WF の正本（sf-tools に一元管理）
    ├── wf-validate.yml
    ├── wf-release.yml
    ├── wf-metasync.yml
    ├── wf-propagate.yml
...
- reusable WF の変更は sf-tools への push だけで全プロジェクトに即時反映される
- caller WF の正本は `sf-tools/templates/` に一元管理する
- `sf-init` は `sf-tools/templates/` からコピーして配布する
- `force-template` リポジトリは WF を持たない（Salesforce プロジェクト雛形のみ）
- 問題になるのは **caller 自体の変更**（secrets の追加削除、job 構造変更など）のみ
