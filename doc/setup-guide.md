...
3. **ブランチ構成の選択**（3 / 2 / 1 階層）
4. **各 Salesforce 組織へのブラウザログイン**（本番・ステージング・開発）
5. **GitHub Classic PAT の発行**（ブラウザで操作・トークンを貼り付け）
6. **Slack Bot Token の取得**（ブラウザで操作・トークンを貼り付け）
7. **Slack チャンネル ID の入力**（`C` で始まる文字列）
