...
補足:
- 本節の reusable workflow 構成は将来的な理想形である
- 現状の実装は `templates/.github/workflows/wf-*.yml` を各プロジェクトへ配布する方式で、`wf-*-reusable.yml` はまだ存在しない
- したがって、現時点の反映経路は `@main` 参照ではなく、`sf-install.sh` によるテンプレート更新である
